...
//Analog sensors on analog pins 0-5
//SD Card Pins:
...
//20210603 - by TSBrownie.  Non-commercial use approved.
...
int sensePin = 0;                                 //Analog pins for sensors
int sensor = 0;                                   //Data from sensor
...
void loop() {
  dataBuff = timeString + ',';
  for (sensePin = 0; sensePin < numSens; sensePin++) {  //Read sensors loop
    GetRTCTime();                                 //Get time from real time clock
    sensor = analogRead(sensePin);                //Read pin, temp store data
    dataBuff += String(sensor);                   //Add data to string
    if (sensePin < (numSens-1)){dataBuff += ",";} //Data divider
  }
